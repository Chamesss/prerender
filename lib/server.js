...
server.init = function (options) {
  ...
  this.options.cacheTTL = this.options.cacheTTL || 3600;

  // Initialize Redis client
  this.redisClient = redis.createClient({
    host: this.options.redisHost || '127.0.0.1',
    port: this.options.redisPort || 6379,
  });
  this.redisClient
    .connect()
    .then(() => util.log('Redis initialized'))
    .catch((err) => util.log('Redis connection error:', err));
  ...
  this.redisClient
    ...
    .then((cachedData) => {
      if (cachedData) {
        ...
        req.prerender.statusCode = data.statusCode;
        req.prerender.content = data.content;
        req.prerender.headers = data.headers;
        req.prerender.fromCache = true;
        util.log('cache hit for', req.prerender.url);
        this.finish(req, res);
        return;
      }
    ...
    .catch((err) => {
      util.log('Redis cache error:', err);
      this.processRequest(req, res);
    });
};

server.processRequest = function (req, res) {
...
server.cacheResult = function (req) {
  if (req.prerender.statusCode === 200 && req.prerender.content) {
    ...
    const ttl = this.options.cacheTTL || 3600; // 1 hour default
    this.redisClient.setEx(cacheKey, ttl, JSON.stringify(data)).catch((err) => {
      util.log('Redis cache set error:', err);
    });
  }
};
...
server.finish = function (req, res) {
  ...
  if (!req.prerender.fromCache && req.prerender.tab) {
  ...
  if (!req.prerender.fromCache) {
    this.removeRequestFromInFlight(req);
  }

  if (
    !req.prerender.fromCache &&
...
server.cacheResult = function (req) {
  // Only cache successful renders (status 200)
  if (req.prerender.statusCode === 200 && req.prerender.content) {
    const cacheKey = req.prerender.url;
    ...
    this.redisClient
      .setEx(cacheKey, this.options.cacheTTL, cacheData)
      .catch((err) => {
        util.log('Redis cache set error:', err);
      });
  }
